...
func (t *transactionService) ResendTransaction(txHash common.Hash) error {
	...
	tx, err := t.signer.SignTx(types.NewTransaction(
	...
	return t.backend.SendTransaction(t.ctx, tx)
}

func (t *transactionService) BoostTransaction(originalTxHash common.Hash, gasPrice *big.Int) error {
	...
	return t.store.Put(storedTransactionKey(originalTxHash), StoredTransaction{
		To:          signedTx.To(),
		Data:        signedTx.Data(),
